...
func (s *VPCClusterScope) GetLoadBalancerHostName() (*string, error) {
	...
	// This function will check if the Load Balancer has an ID set, use that to check Status for the Load Balancer's hostname. Or, if Name is non-empty, lookup the current Load Balancer status for hostname. Other cases result in errors.
	getHostName := func(loadBalancer infrav1beta2.VPCLoadBalancerSpec) (*string, error) {
		// Attempt to retrieve the Load Balancer's hostname from Status by ID.
		if loadBalancer.ID != nil {
			if lb, ok := s.NetworkStatus().LoadBalancers[*loadBalancer.ID]; ok {
				return lb.Hostname, nil
			}
			// If the Load Balancer isn't in Status, assume it is not ready/reconciled.
			return nil, nil
		}
		...
		if loadBalancer.Name == "" {
			return nil, fmt.Errorf("error load balancer spec has no id or name defined")
		}
		// Lookup current Load Balancer hostname, via its Status, based on name, as ID was already found to be nil.
		loadBalancerStatus, err := s.getLoadBalancer(loadBalancer)
		if err != nil {
			return nil, fmt.Errorf("error retrieving load balancer status: %w", err)
		} else if loadBalancerStatus == nil {
			// If no Load Balancer Status is found, return an error.
			return nil, fmt.Errorf("error no load balancer status available for %s", loadBalancer.Name)
		}
		return loadBalancerStatus.Hostname, nil
	}
	...
	for _, loadBalancer := range s.NetworkSpec().LoadBalancers {
		...
		// Attempt to return the hostname for the public Load Balancer, via Status by ID, or lookup by Name.
		return getHostName(loadBalancer)
	}
