...
func (m *MachineScope) CreateMachine() (*vpcv1.Instance, error) { //nolint: gocyclo
	...
	// Find subnet ID from Cluster Network Status, if possible.
	...
	} else if subnet, ok := m.IBMVPCCluster.Status.Network.WorkerSubnets[m.IBMVPCMachine.Spec.PrimaryNetworkInterface.Subnet]; ok {
		subnetIdentity.ID = ptr.To(subnet.ID)
	} else {
...
func (m *MachineScope) getLoadBalancerID(loadBalancer *infrav1beta2.VPCResource) (*string, error) {
	...
	if loadBalancer.ID != nil {
		...
		if err != nil {
			...
			return nil, fmt.Errorf("error unable to find laod balancer id with name: %s", *loadBalancer.Name)
		}
...
func (m *MachineScope) checkVPCLoadBalancerPoolMemberExists(poolMember infrav1beta2.VPCLoadBalancerBackendPoolMember, internalIP *string) (*vpcv1.LoadBalancerPoolMember, error) {
	...
	for _, member := range poolMembers.Members {
		if target, ok := member.Target.(*vpcv1.LoadBalancerPoolMemberTarget); ok {
			// Verify the target address matches the Machine internal IP.
...
func (m *MachineScope) createVPCLoadBalancerPoolMember(poolMember infrav1beta2.VPCLoadBalancerBackendPoolMember, internalIP *string) (bool, error) {
	...
	options := &vpcv1.CreateLoadBalancerPoolMemberOptions{
		...
		// TODO(cjschaef): We likely want to use the Machine's IP instead of ID.
	...
	// TODO(cjschaef): Weight only affects weightroundrobin algorithm on a LB. We may wish to validate this, unless API ignores this field for other algorithms (and it doesn't matter if we provide it).
	...
	// Update Machine Status with pool member.
	if len(m.IBMVPCMachine.Status.LoadBalancerPoolMembers) > 0 {
		for _, member := range m.IBMVPCMachine.Status.LoadBalancerPoolMembers {
			if *member.LoadBalancer.ID == *loadBalancerID && *member.Pool.ID == *loadBalancerBackendPoolID && member.Port == poolMember.Port {
				err := fmt.Errorf("error created load balancer pool member already found in machine status")
				m.Logger.Error(err, "machineName", m.IBMVPCMachine.Name, "loadBalancerID", *loadBalancerID, "loadBalancerBackendPoolID", *loadBalancerBackendPoolID, "loadBalancerPoolMemberID", *loadBalancerPoolMember.ID)
				return false, err
			}
		}
	}

	// Otherwise, add the new pool member details to the Machine Status.
...
func (m *MachineScope) SetAddresses(instance *vpcv1.Instance) {
	...
	// Currently, only the single network interface designed by a subnet, is expected for the Instance (as it's primary/internal IP).
	addresses = append(addresses, corev1.NodeAddress{
