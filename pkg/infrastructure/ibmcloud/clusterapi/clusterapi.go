...
func (p Provider) InfraReady(ctx context.Context, in clusterapi.InfraReadyInput) error {
	...
	if ibmcloudCluster.Status.Network.LoadBalancers == nil || len(ibmcloudCluster.Status.Network.LoadBalancers) == 0 {
	...
	for lbID, lb := range ibmcloudCluster.Status.Network.LoadBalancers {
		...
		lbDetails, err := client.GetLoadBalancer(context.TODO(), lbID)
		if err != nil {
		...
		err = metadata.CreateDNSRecord(context.TODO(), in.InstallConfig.Config.ClusterDomain(), lbDetails)
		if err != nil {
