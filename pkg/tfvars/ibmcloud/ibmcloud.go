...
type config struct {
	...
	PreexistingVPC           string          `json:"ibmcloud_preexisting_vpc,omitempty"`
...
func TFVars(sources TFVarsSources) ([]byte, error) {
	...
	cfg := &config{
		...
		PreexistingVPC:           sources.VPCPermitted,
...
func CreateEndpointJSON(endpoints []configv1.IBMCloudServiceEndpoint, region string) (string, error) {
	...
	for _, endpoint := range endpoints {
		endpointVar, ok := ibmcloudtypes.IBMCloudServiceOverride[strings.ToLower(endpoint.Name)]
		if !ok {
			return "", errors.New("unable to find environment variable for service endpoint %s", endpoint.Name)
		}
	...
	if len(endpointContents) > 0 {
		jsonString, err := json.Marshal(endpointContents)
		if err != nil {
			return "", errors.Wrap(err, "failure building service endpoint override JSON data")
		}
	...
	return "", errors.New("unsupported endpoints provided: %s", endpoints)
}
