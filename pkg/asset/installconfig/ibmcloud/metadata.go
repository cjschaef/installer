...
func NewMetadata(baseDomain string, region string, controlPlaneSubnets []string, computeSubnets []string, endpoints []configv1.IBMCloudServiceEndpoint) *Metadata {
	return &Metadata{
...
func (m *Metadata) Client() (API, error) {
	...
	if len(m.serviceEndpoints) > 0 {
		client, err = NewClientEndpointOverride(m.serviceEndpoints)
	} else {
