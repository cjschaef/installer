...
// NewClientEndpointOverride initializes a client with provided endpoint overrides
func NewClientEndpointOverride(endpoints []configv1.IBMCloudServiceEndpoint) (*Client, error) {
	apiKey := os.Getenv("IC_API_KEY")
	...
	for _, endpoint := range endpoints {
		if strings.ToLower(endpoint.Name) == ibmcloudtypes.IBMCloudServiceIAM {
	...
	if err := client.loadSDKServices(): err != nil {
		return nil, errors.Wrap(err, "failed to load overridden IBM SDK services")
	}
...
func (c *Client) GetAuthenticatorAPIKeyDetails(ctx context.Context) (*iamidentityv1.APIKey, error) {
	...
	if overrideURL := ibmcloudtypes.CheckServiceEndpointOverride(ibmcloudtypes.IBMCloudServiceIAM, c.serviceEndpoints); overrideURL != "" {
		newIamIdentityV1Options.URL = overrideURL
	}
...
func (c *Client) GetDNSInstancePermittedNetworks(ctx context.Context, dnsID string, dnsZone string) ([]string, error) {
	...
	if overrideURL := ibmcloudtypes.CheckServiceEndpointOverride(ibmcloudtypes.IBMCloudServiceDNSServices, c.serviceEndpoints); overrideURL != "" {
...
func (c *Client) GetDNSRecordsByName(ctx context.Context, crnstr string, zoneID string, recordName string) ([]dnsrecordsv1.DnsrecordDetails, error) {
	...
	// dnsrecordsv1 is provided via IBM Cloud DNS Services endpoint
	if overrideURL := ibmcloudtypes.CheckServiceEndpointOverride(ibmcloudtypes.IBMCloudServiceCIS, c.serviceEndpoints); overrideURL != "" {
		options.URL = overrideURL
	}
...
func (c *Client) getCISDNSZones(ctx context.Context) ([]responses.DNSZoneResponse, error) {
	...
	for _, instance := range listResourceInstancesResponse.Resources {
		...
		if overrideURL := ibmcloudtypes.CheckServiceEndpointOverride(ibmcloudtypes.IBMServiceCIS, c.serviceEndpoints); overrideURL != "" {
			options.URL = overrideURL
		}
...
func (c *Client) loadResourceManagementAPI() error {
	...
	if overrideURL := ibmcloudtypes.CheckServiceEndpointOverride(ibmcloudtypes.IBMCloudServiceResourceManager, c.serviceEndpoints); overrideURL != "" {
		options.URL = overrideURL
	}
...
func (c *Client) loadResourceControllerAPI() error {
	...
	if overrideURL := ibmcloudtypes.CheckServiceEndpointOverride(ibmcloudtypes.IBMCloudServiceResourceController, c.serviceEndpoints); overrideURL != "" {
		options.URL = overrideURL
	}
...
func (c *Client) loadVPCV1API() error {
	...
	if overrideURL := ibmcloudtypes.CheckServiceEndpointOverride(ibmcloudtypes.IBMCloudServiceVPC, c.serviceEndpoints); overrideURL != "" {
		options.URL = overrideURL
	}
