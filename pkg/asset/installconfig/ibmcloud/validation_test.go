...
func TestValidateServiceEndpoints(t *testing.T) {
	cases := []struct {
		...
		{
			name: "single unexpected override service endpoint",
			edits: editFunctions{
				func(ic *types.InstallConfig) {
					ic.Platform.IBMCloud.ServiceEndpoints = []configv1.IBMCloudServiceEndpoint{
						{
		...
		},
		{
			...
			errorMsg: `\Q[platform.ibmcloud.serviceEndpoints[1].url: Invalid value: "/invalid/uri/format": Head "/invalid/uri/format": unsupported protocol scheme "", platform.ibmcloud.serviceEndpoints[2].name: Duplicate value: "cos"]\E`,
		},
		{
			...
			errorMsg: `\Qplatform.ibmcloud.serviceEndpoints[2].name: Duplicate value: "cos"\E`,
		},
	}
